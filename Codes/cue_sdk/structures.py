...
class CorsairDeviceInfo(namedtuple('CorsairDeviceInfo', _CorsairDeviceInfo.__slots__)):
    ...
    def __new__(cls, dev_info):
        ...
        """
        dev_type = CDT(dev_info.type) # my MM800 is not recognized as a CDT dev type. So it must be switched.
    ...
    #   FROM       return super(CorsairDeviceInfo, cls).__new__(cls, dev_type ,dev_model, dev_pl, dev_ll, dev_cm)
    #       this is the original one from the lib developer. However, it seems not to include mousepad as a CDT dev type. so i switiched it into simplly dev_model.
    #       dev_type -> dev_model it works as well here.
